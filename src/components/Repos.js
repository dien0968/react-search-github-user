...
const Repos = () => {
  ...
  const languages = repos.reduce((total, item) => {
    ...
    if(!language) return total //Trả về Objcet rỗng
  ...
  const mostUser = Object.values(languages).sort((a,b) => {
    return b.value - a.value
  }).slice(0, 5)

  const mostStar = Object.values(languages).sort((a,b) => {
    return b.star - a.star
  }).map((item) => {
    return {...item, value: item.star}
  }).slice(0, 5)

  // console.log('languagessss', languages)

  let {stars, forks} = repos.reduce((total, item) => {
    const {stargazers_count, forks, name} = item
    
    total.stars[stargazers_count] = {label: name, value: stargazers_count}
    // console.log('total', total.stars[stargazers_count])
